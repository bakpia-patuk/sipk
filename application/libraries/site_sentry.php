...
class Site_sentry 
{
	...
	function login_routine()
	{
		...
		foreach($query->result() as $row)
		{
			if ($row->email == $username && $this->obj->encrypt->decode($row->password) == $password && $row->access_level != 0)
			{
				...
				$this->obj->db->update('users'); // if they have successfully logged in, we don't need that anymore
			}
		}
		...
		{
            $this->obj->session->set_flashdata('message', array('type' => 'error' , 'message' => 'Username and password do not match or you do not have an account yet.'));
    ...
    {
        return $this->obj->session->userdata('user_id');
    }

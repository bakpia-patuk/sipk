<script type="text/javascript">
    ...
    */
    ...
    window.addEvent('domready', function() {
        new Fx.Accordion(
            $$('div#kursus-sliders.pane-sliders > .panel > h3.pane-toggler'), 
            $$('div#kursus-sliders.pane-sliders > .panel > div.pane-slider'), {
                onActive: function(toggler, i) {
                    toggler.addClass('pane-toggler-down');
                    toggler.removeClass('pane-toggler');
                    i.addClass('pane-down');
                    i.removeClass('pane-hide');
                    Cookie.write('jpanesliders_kursus-sliders', $$('div#kursus-sliders.pane-sliders > .panel > h3').indexOf(toggler));
                },
                onBackground: function(toggler, i) {
                    ...
                    if ($$('div#kursus-sliders.pane-sliders > .panel > h3').length == $$('div#kursus-sliders.pane-sliders > .panel > h3.pane-toggler').length)
                        Cookie.write('jpanesliders_kursus-sliders',-1);
                },
                duration: 300,
                opacity: false,
                alwaysHide: true
            ...
            Joomla.submitbutton = function(task)
            {
                switch (task) {
                    case 'kursus.apply':
                        ...
                        jform_task.value = 'kursus.apply';
                        var jform_form = document.getElementById('kursus-form');
                        jform_form.action = "<?php echo site_url('admin/kursus/add?'); ?>";
                        Joomla.submitform(task, document.getElementById('kursus-form'));
                        break;
                    case 'kursus.save':
                        var form = document.getElementById('kursus-form');
                        form.action = "<?php echo site_url('admin/kursus/add'); ?>";
                        Joomla.submitform(task, document.getElementById('kursus-form'));
                        break;
                ...
                if (task == 'kursus.cancel' || document.formvalidator.isValid(document.id('kursus-form'))) {
                    Joomla.submitform(task, document.getElementById('kursus-form'));
                }
            }
		</script>
        <?php echo validation_errors(); ?>
		<form action="<?php echo site_url('admin/kursus'); ?>" method="post" name="adminForm" id="kursus-form" class="form-validate">
			<div class="width-60 fltlft">
				<fieldset class="adminform">
					<legend>
						<?php
							if ($is_new)
								echo "Bimbel/Kursus/Test Baru";
							else
								echo "Edit Bimbel/Kursus/Test";
						?>
					</legend>
					<ul class="adminformlist">
						...
						<li>
                            ...
                            <select id="jform_state" name="jform[state]" class="inputbox" size="1">
                                <?php
                                    foreach ($kursus_state AS $key => $value) {
                                        if ($key == $data->state)
                        ...
                        <li>
                            ...
                            <select id="jform_kategori" name="jform[kategori]" class="inputbox" size="1">
                                <?php
                                    foreach ($kursus_kategori AS $key => $value) {
                        ...
                        <li>
                            ...
                            <select id="jform_wilayah" name="jform[wilayah]" class="inputbox" size="1">
                                <?php
                                    foreach ($kursus_wilayah AS $key => $value) {
                                        if ($key == $data->wilayah)
                                            echo "<option value='$key' selected='selected'>$value&nbsp;&nbsp;</option>";
                                        else
                                            echo "<option value='$key'>$value&nbsp;&nbsp;</option>";
			...
			<div class="width-40 fltrt">
				<div id="kursus-sliders" class="pane-sliders">

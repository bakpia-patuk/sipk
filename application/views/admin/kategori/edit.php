<script type="text/javascript">
    ...
    */
    ...
    window.addEvent('domready', function() {
        new Fx.Accordion(
            $$('div#kategori-sliders.pane-sliders > .panel > h3.pane-toggler'), 
            $$('div#kategori-sliders.pane-sliders > .panel > div.pane-slider'), {
                onActive: function(toggler, i) {
                    toggler.addClass('pane-toggler-down');
                    toggler.removeClass('pane-toggler');
                    i.addClass('pane-down');
                    i.removeClass('pane-hide');
                    Cookie.write('jpanesliders_kategori-sliders', $$('div#kategori-sliders.pane-sliders > .panel > h3').indexOf(toggler));
                },
                onBackground: function(toggler, i) {
                    ...
                    if ($$('div#kategori-sliders.pane-sliders > .panel > h3').length == $$('div#kategori-sliders.pane-sliders > .panel > h3.pane-toggler').length)
                        Cookie.write('jpanesliders_kategori-sliders',-1);
                },
                duration: 300,
                opacity: false,
                alwaysHide: true
			...
			Joomla.submitbutton = function(task)
            {
                switch (task) {
                    case 'kategori.apply':
                        ...
                        jform_task.value = 'kategori.apply';
                        var jform_form = document.getElementById('kategori-form');
                        jform_form.action = "<?php echo site_url('admin/kategori/add'); ?>";
                        Joomla.submitform(task, document.getElementById('kategori-form'));
                        break;
                    case 'kategori.save':
                        var form = document.getElementById('kategori-form');
                        form.action = "<?php echo site_url('admin/kategori/add'); ?>";
                        Joomla.submitform(task, document.getElementById('kategori-form'));
                        break;
                ...
                if (task == 'kategori.cancel' || document.formvalidator.isValid(document.id('kategori-form'))) {
                    Joomla.submitform(task, document.getElementById('kategori-form'));
                }
            }
		</script>
        <?php echo validation_errors(); ?>
		<form action="<?php echo site_url('admin/kategori'); ?>" method="post" name="adminForm" id="kategori-form" class="form-validate">
			<div class="width-100">
				<fieldset class="adminform">
					...
					<ul class="adminformlist">
						<li>
							<label id="jform_nama-lbl" for="jform_nama" class="hasTip required" title="">Nama Kategori<span class="star">&nbsp;*</span></label>
                            <input type="hidden" name="jform[id]" id="jform_id" value="<?php echo $data->id; ?>">
							<input name="jform[kategori]" id="jform_nama" class="inputbox required" size="54" maxlength="100" type="text" value="<?php echo htmlentities($data->kategori); ?>" >
						</li>
					</ul>
					<div class="clr"></div>
				</fieldset>
